...
try:
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.bitconchioAccount)
    ...
    contractDir="unittests/contracts/bccio.token"
    wasmFile="bccio.token.wasm"
    abiFile="bccio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getBitconchAccountFromDb(currencyAccount.name)
        ...
        if abiName != "account" or abiActionName != "close" or abiType != "close":
            errorExit("FAILURE - get BITCONCH account failed", raw=True)
    ...
    account=node.getBitconchAccount(defproduceraAccount.name, exitOnError=True)
    ...
    if not enableMongo:
        ...
        rawAccount=node.getBitconchAccount(defproduceraAccount.name, exitOnError=True, returnType=ReturnType.raw)
