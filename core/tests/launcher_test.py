...
try:
    ...
    if not dontLaunch:
        ...
        killBitconchInstances=False

        print("Stand up walletd")
        if walletMgr.launch() is False:
            cmdError("%s" % (WalletdName))
            errorExit("Failed to stand up bitconch walletd.")
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.bitconchioAccount,cluster.defproduceraAccount])
    ...
    # create accounts via bccio as otherwise a bid is needed 
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.bitconchioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.bitconchioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.bitconchioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.bitconchioAccount, buyRAM=1000000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.bitconchioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.bitconchioAccount, buyRAM=1000000, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountBitconchBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountBitconchBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    if cluster.bounce("00,01") is False:
        ...
        errorExit("Failed to bounce bitconch node.")

    Print("Taking down node #02")
    if cluster.down("02") is False:
        cmdError("launcher down command")
        errorExit("Failed to take down bitconch node.")
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killBitconchInstances, killWallet, keepLogs, killAll, dumpErrorDetails)
