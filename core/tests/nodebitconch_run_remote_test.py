...
###############################################################
# nodebitconch_run_remote_test
#  Tests remote capability of the nodebitconch_run_test. Test will setup cluster and pass nodes info to nodebitconch_run_test. E.g.
#  nodebitconch_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodebitconch_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodebitconch", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killBitconchInstances, False, False, killAll, dumpErrorDetails)
