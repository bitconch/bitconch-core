...
try:
    ...
    maxRAMValue=1010
    extraNodebitconchArgs=" %s %d %s %d " % (minRAMFlag, minRAMValue, maxRAMFlag, maxRAMValue)
    if cluster.launch(onlyBios=False, pnodes=totalNodes, totalNodes=totalNodes, totalProducers=totalNodes, extraNodebitconchArgs=extraNodebitconchArgs, useBiosBootFile=False) is False:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.bitconchioAccount])
    ...
    # create accounts via bitconchio as otherwise a bid is needed
    for account in accounts:
        Print("Create new account %s via %s" % (account.name, cluster.bitconchioAccount.name))
        trans=nodes[0].createInitializeAccount(account, cluster.bitconchioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
        transferAmount="70000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.bitconchioAccount.name, account.name))
        nodes[0].transferFunds(cluster.bitconchioAccount, account, transferAmount, "test transfer")
    ...
    Print("Create new account %s via %s" % (contractAccount.name, cluster.bitconchioAccount.name))
    trans=nodes[0].createInitializeAccount(contractAccount, cluster.bitconchioAccount, stakedDeposit=500000, waitForTransBlock=False, stakeNet=50000, stakeCPU=50000, buyRAM=50000, exitOnError=True)
    transferAmount="90000000.0000 {0}".format(CORE_SYMBOL)
    Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.bitconchioAccount.name, contractAccount.name))
    nodes[0].transferFunds(cluster.bitconchioAccount, contractAccount, transferAmount, "test transfer")
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                       Print("Failed to push create action to bitconchio contract for %d consecutive times, looks like nodebitconch already exited." % (timeOutCount))
                       keepProcessing=False
                       break
                    Print("Failed to push create action to bitconchio contract. sleep for 60 seconds")
                    time.sleep(60)
                else:
                    timeOutCount=0
                time.sleep(1)
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful=testSuccessful, killBitconchInstances=killBitconchInstances, killWallet=killWallet, keepLogs=keepLogs, cleanRun=killAll, dumpErrorDetails=dumpErrorDetails)
