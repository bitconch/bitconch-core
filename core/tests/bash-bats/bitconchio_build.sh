...
SCRIPT_LOCATION="scripts/bitconchio_build.sh"
TEST_LABEL="[bitconchio_build]"

###################################################################
# ON MAC YOU NEED TO FULLY UNINSTALL BITCONCHIO BEFORE THESE WILL PASS #
###################################################################
...
@test "${TEST_LABEL} > Testing Arguments & Options" {
    ...
    if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
        ...
        run bash -c "printf \"y\ny\nn\nn\n\" | ./scripts/bitconchio_build.sh"
        [[ ! -z $(echo "${output}" | grep "BITCONCHIO compiler checks require the 'which'") ]] || exit
    fi
    ...
    run bash -c "./bitconchio_build.sh -y -P"
    ...
    # lack of -i
    [[ ! -z $(echo "${output}" | grep "BITCONCHIO_INSTALL_DIR: ${HOME}/bitconchio/${BITCONCHIO_VERSION}") ]] || exit
    ## -o
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -o Debug -P"
    [[ ! -z $(echo "${output}" | grep "CMAKE_BUILD_TYPE: Debug") ]] || exit
    ## -s
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -s BITCONCH2 -P"
    [[ ! -z $(echo "${output}" | grep "CORE_SYMBOL_NAME: BITCONCH2") ]] || exit
    ## -b
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -b /test -P"
    [[ ! -z $(echo "${output}" | grep "BOOST_LOCATION: /test") ]] || exit
    ## -i
    run bash -c "printf \"y\ny\nn\nn\n\"| ./$SCRIPT_LOCATION -i /NEWPATH -P"
    [[ ! -z $(echo "${output}" | grep "BITCONCHIO_INSTALL_DIR: /NEWPATH") ]] || exit
    [[ ! -z $(echo "${output}" | grep "TEMP_DIR: ${HOME}/tmp") ]] || exit
        ...
        [[ ! -z $(echo "${output}" | grep "BITCONCHIO_INSTALL_DIR: $TEMP_DIR/NEWPATH") ]] || exit
