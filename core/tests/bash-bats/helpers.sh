...
@test "${TEST_LABEL} > previous install prompt" {
  ...
  [[ -z $(echo "${output}") ]] || exit
  # Exists, prompt
  mkdir -p $BITCONCHIO_INSTALL_DIR
  run previous-install-prompt
  [[ ! -z $(echo "${output}" | grep "BITCONCHIO has already been installed into ${BITCONCHIO_INSTALL_DIR}") ]] || exit
  rm -rf $BITCONCHIO_INSTALL_DIR
}
