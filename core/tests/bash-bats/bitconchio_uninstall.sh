...
SCRIPT_LOCATION="scripts/bitconchio_uninstall.sh"
TEST_LABEL="[bitconchio_uninstall]"
...
@test "${TEST_LABEL} > Testing user prompts" {
  ...
  [[ $output =~ " - BITCONCHIO Removal Complete" ]] || exit
  ## First no shows "Cancelled..."
  run bash -c "echo \"n\" | ./$SCRIPT_LOCATION"
  [[ "${output##*$'\n'}" =~ "Cancelled BITCONCHIO Removal!" ]] || exit
...
@test "${TEST_LABEL} > -i" {
  run ./$SCRIPT_LOCATION -y -i bitconchiotest
  [[ $output =~ .*/bitconchiotest ]] || exit
  ([[ ! $output =~ "Library/Application\ Support/bitconchio" ]] && [[ ! $output =~ ".local/share/bitconchio" ]]) || exit
  [[ ! $output =~ "BITCONCHIO Removal Complete" ]] || exit
}
