...
try:
    ...
    for nodeNum in range(txnGenNodeNum, txnGenNodeNum+startedNonProdNodes):
        specificExtraNodebitconchArgs[nodeNum]="--plugin bitconchio::txn_test_gen_plugin --txn-test-gen-account-prefix txntestacct"
    ...
    if cluster.launch(prodCount=prodCount, onlyBios=False, pnodes=pnodes, totalNodes=totalNodes, totalProducers=pnodes*prodCount,
                      useBiosBootFile=False, specificExtraNodebitconchArgs=specificExtraNodebitconchArgs, unstartedNodes=catchupCount, loadSystemContract=False) is False:
    ...
    txnGenNodes[0].txnGenCreateTestAccounts(cluster.bitconchioAccount.name, cluster.bitconchioAccount.activePrivateKey)
