...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="bitconchio-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kbitconchd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="BITCONCHtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kbitconchd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: bitconch server host
        port: bitconch server port
        walletHost: bitconch wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures bitconch mongo plugin
        ...
        self.bitconchioAccount=self.defProducerAccounts["bccio"]= Account("bccio")
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraNodebitconchArgs=None, useBiosBootFile=True, specificExtraNodebitconchArgs=None, onlySetProds=False,
        ...
        extraNodebitconchArgs: string of arguments to pass through to each nodoes instance (via --nodebitconch flag on launcher)
        ...
        specificExtraNodebitconchArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodebitconch flags on launcher), example: { "5" : "--plugin bitconchio::test_control_api_plugin" }
        ...
        loadSystemContract: indicate whether the bccio.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.BitconchServerName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodebitconch default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f %s --unstarted-nodes %s" % (
            Utils.BitconchLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            producerFlag, unstartedNodes)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodebitconchArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodebitconchArgs += " --plugin bitconchio::wallet_api_plugin"
        if self.enableMongo:
            nodebitconchArgs += " --plugin bitconchio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodebitconchArgs is not None:
            assert(isinstance(extraNodebitconchArgs, str))
            nodebitconchArgs += extraNodebitconchArgs
        if Utils.Debug:
            nodebitconchArgs += " --contracts-console"
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodebitconchArgs += " --plugin bitconchio::producer_api_plugin"

        if nodebitconchArgs:
            cmdArr.append("--nodebitconch")
            cmdArr.append(nodebitconchArgs)

        if specificExtraNodebitconchArgs is not None:
            assert(isinstance(specificExtraNodebitconchArgs, dict))
            for nodeNum,arg in specificExtraNodebitconchArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-nodebitconch")
                cmdArr.append(arg)
        ...
        if associatedNodeLabels is not None:
            for nodeNum,label in associatedNodeLabels.items():
                ...
                cmdArr.append(str(nodeNum))
                cmdArr.append("--spcfc-inst-nodebitconch")
                cmdArr.append(path)
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.BitconchServerName, startedNodes, len(nodes)))
            return False
        ...
        self.bitconchioAccount=self.defProducerAccounts["bccio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.BitconchClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.BitconchClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on bitconch server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextBitconchIdx: %d, n: %d" % (nextBitconchIdx, n))
                nextBitconchIdx=(nextBitconchIdx + 1)%count
                if not self.nodes[nextBitconchIdx].killed:
                    #Utils.Print("nextBitconchIdx: %d" % (nextBitconchIdx))
            ...
            #Utils.Print("nextBitconchIdx: %d, count: %d" % (nextBitconchIdx, count))
            node=self.nodes[nextBitconchIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.BitconchServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on bitconch node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getBitconchBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.bitconchioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse bitconchio private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by bitconchio-launcher."""
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/bccio.bios",
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/bccio.bios"
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down bitconch cluster.")
            return None
        ...
        bitconchioName="bccio"
        bitconchioKeys=producerKeys[bitconchioName]
        bitconchioAccount=Account(bitconchioName)
        bitconchioAccount.ownerPrivateKey=bitconchioKeys["private"]
        bitconchioAccount.ownerPublicKey=bitconchioKeys["public"]
        bitconchioAccount.activePrivateKey=bitconchioKeys["private"]
        bitconchioAccount.activePublicKey=bitconchioKeys["public"]
        producerKeys.pop(bitconchioName)

        if not self.walletMgr.importKey(bitconchioAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (bitconchioName))
            return None

        initialFunds="1000000.0000 {0}".format(CORE_SYMBOL)
        Utils.Print("Transfer initial fund %s to individual accounts." % (initialFunds))
        trans=None
        contract="bccio.token"
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"bccio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init bccio transfer")
            opts="--permission bccio@active"
            if name != "bccio":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from bccio.token to %s." % (name))
                    return None
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        ...
        trans=biosNode.publishContract(bitconchioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, bitconchioAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission bccio@active"
                    myTrans=biosNode.pushMessage("bccio", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("bccio", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but bitconchio).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "bccio"
        ...
        bitconchioTokenAccount=copy.deepcopy(bitconchioAccount)
        bitconchioTokenAccount.name="bccio.token"
        trans=biosNode.createAccount(bitconchioTokenAccount, bitconchioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (bitconchioTokenAccount.name))
            return None

        bitconchioRamAccount=copy.deepcopy(bitconchioAccount)
        bitconchioRamAccount.name="bccio.ram"
        trans=biosNode.createAccount(bitconchioRamAccount, bitconchioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (bitconchioRamAccount.name))
            return None

        bitconchioRamfeeAccount=copy.deepcopy(bitconchioAccount)
        bitconchioRamfeeAccount.name="bccio.ramfee"
        trans=biosNode.createAccount(bitconchioRamfeeAccount, bitconchioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (bitconchioRamfeeAccount.name))
            return None

        bitconchioStakeAccount=copy.deepcopy(bitconchioAccount)
        bitconchioStakeAccount.name="bccio.stake"
        trans=biosNode.createAccount(bitconchioStakeAccount, bitconchioAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (bitconchioStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(bitconchioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=bitconchioTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (bitconchioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to bitconchio contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (bitconchioAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (bitconchioAccount.name)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to bitconchio contract.")
            return None
        ...
        Utils.Print("Verify bitconchio issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountBitconchBalanceStr(bitconchioAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (bitconchioAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (bitconchioAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (bitconchioTokenAccount.name, name))
                return None
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (bitconchioAccount.name)
            trans=biosNode.pushMessage(bitconchioAccount.name, action, data, opts)
    ...
    @staticmethod
    def pgrepBitconchServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.BitconchServerName)
    ...
    @staticmethod
    def pgrepBitconchServerPattern(nodeInstance):
        dataLocation=Utils.getNodeDataDir(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of BitconchInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepBitconchServers(timeout)
        if psOut is None:
    ...
    def discoverLocalNode(self, nodeNum, psOut=None, timeout=None):
        ...
        if psOut is None:
            Utils.Print("ERROR: No nodes discovered.")
            return None
        pattern=Cluster.pgrepBitconchServerPattern(nodeNum)
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.BitconchServerName, pattern))
            return None
    ...
    def discoverBiosNode(self, timeout=None):
        psOut=Cluster.pgrepBitconchServers(timeout=timeout)
        pattern=Cluster.pgrepBitconchServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.BitconchServerName, pattern))
    ...
    # Kills a percentange of Bitconch instances starting from the tail and update bitconchInstanceInfos state
    def killSomeBitconchInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.BitconchServerName, killSignal))
    ...
    def relaunchBitconchInstances(self, cachePopen=False):
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodebitconch instances. allInstances will kill all nodebitconch instances running on the system."""
        cmd="%s -k 9" % (Utils.BitconchLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down bitconch cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the bitconch server
            cmd="pkill -9 %s" % (Utils.BitconchServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down bitconch cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces nodebitconch instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down nodebitconch instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)
