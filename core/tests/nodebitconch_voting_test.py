...
try:
    ...
    for account in accounts:
        ...
        trans=node.createInitializeAccount(account, cluster.bitconchioAccount, stakedDeposit=0, waitForTransBlock=False, stakeNet=1000, stakeCPU=1000, buyRAM=1000, exitOnError=True)
        transferAmount="100000000.0000 {0}".format(CORE_SYMBOL)
        Print("Transfer funds %s from account %s to %s" % (transferAmount, cluster.bitconchioAccount.name, account.name))
        node.transferFunds(cluster.bitconchioAccount, account, transferAmount, "test transfer")
