...
def restartNode(node: Node, nodeId, chainArg=None, addOrSwapFlags=None, nodebitconchPath=None):
    if not node.killed:
        node.kill(signal.SIGTERM)
    isRelaunchSuccess = node.relaunch(nodeId, chainArg, addOrSwapFlags=addOrSwapFlags,
                                      timeout=5, cachePopen=True, nodebitconchPath=nodebitconchPath)
...
try:
    ...
    assert cluster.launch(pnodes=4, totalNodes=4, prodCount=1, totalProducers=4,
                          extraNodebitconchArgs=" --plugin bitconchio::producer_api_plugin ",
    ...
    # Finally, when we restart the 4th node with the version of nodebitconch that supports protocol feature,
    ...
    oldNode.relaunch(oldNodeId, chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, nodebitconchPath="programs/nodebitconch/nodebitconch")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, oldNodeId, chainArg="--replay", nodebitconchPath="programs/nodebitconch/nodebitconch")
