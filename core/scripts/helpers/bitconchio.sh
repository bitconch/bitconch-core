...
if [[ -z "${NAME}" ]]; then
    ...
    else echo " ${COLOR_RED}- BITCONCHIO is not supported for your Architecture!${COLOR_NC}" && exit 1
    fi
fi

# Setup yum and apt variables
if [[ $NAME =~ "Amazon Linux" ]] || [[ $NAME == "CentOS Linux" ]]; then
    if ! YUM=$( command -v yum 2>/dev/null ); then echo "${COLOR_RED}YUM must be installed to compile BITCONCHIO${COLOR_NC}" && exit 1; fi
elif [[ $NAME == "Ubuntu" ]]; then
    if ! APTGET=$( command -v apt-get 2>/dev/null ); then echo "${COLOR_RED}APT-GET must be installed to compile BITCONCHIO${COLOR_NC}" && exit 1; fi
fi
...
function setup() {
    if $VERBOSE; then
        ...
        echo "BITCONCHIO_INSTALL_DIR: ${BITCONCHIO_INSTALL_DIR}"
...
function ensure-which() {
  if ! which ls &>/dev/null; then
    while true; do
      [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}BITCONCHIO compiler checks require the 'which' package: Would you like for us to install it? (y/n)?${COLOR_NC}" && read -p " " PROCEED
      echo ""
      case $PROCEED in
...
function install-directory-prompt() {
    if [[ -z $INSTALL_LOCATION ]]; then
        echo "No installation location was specified. Please provide the location where BITCONCHIO is installed."
        while true; do
            [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to use the default location? ${BITCONCHIO_INSTALL_DIR}? (y/n)${COLOR_NC}" && read -p " " PROCEED
            echo ""
            case $PROCEED in
                "" )
                echo "What would you like to do?";;
                ...
                break;;
                1 | false | [Nn]* )
                printf "Enter the desired installation location." && read -p " " BITCONCHIO_INSTALL_DIR;
                export BITCONCHIO_INSTALL_DIR;
                break;;
                * ) echo "Please type 'y' for yes or 'n' for no.";;
            esac
        done
    else
        # Support relative paths : https://github.com/BITCONCHIO/bitconch/issues/7560
        [[ ! $INSTALL_LOCATION =~ ^\/ ]] && export INSTALL_LOCATION="${CURRENT_WORKING_DIR}/$INSTALL_LOCATION"
        export BITCONCHIO_INSTALL_DIR="$INSTALL_LOCATION"
    fi
    . ./scripts/.build_vars
    echo "BITCONCHIO will be installed to: ${BITCONCHIO_INSTALL_DIR}"
}

function previous-install-prompt() {
  if [[ -d $BITCONCHIO_INSTALL_DIR ]]; then
    echo "BITCONCHIO has already been installed into ${BITCONCHIO_INSTALL_DIR}... It's suggested that you bitconchio_uninstall.sh before re-running this script."
    while true; do
...
function resources() {
    echo "${COLOR_CYAN}BITCONCHIO website:${COLOR_NC} https://bitconch.io"
    echo "${COLOR_CYAN}BITCONCHIO Telegram channel:${COLOR_NC} https://t.me/BITCONCHProject"
    echo "${COLOR_CYAN}BITCONCHIO resources:${COLOR_NC} https://bitconch.io/resources/"
    echo "${COLOR_CYAN}BITCONCHIO Stack Exchange:${COLOR_NC} https://bitconchio.stackexchange.com"
}

function print_supported_linux_distros_and_exit() {
   echo "On Linux the BITCONCHIO build script only supports Amazon, Centos, and Ubuntu."
...
function ensure-compiler() {
    ...
    if $PIN_COMPILER || [[ -f $CLANG_ROOT/bin/clang++ ]]; then
        ...
        if [[ $CXX =~ "clang" ]] || [[ $COMPILER_TYPE =~ "clang" ]]; then
            ...
            if [[ $NO_CPP17 == false ]]; then # https://github.com/BITCONCHIO/bitconch/issues/7402
                while true; do
                    echo "${COLOR_YELLOW}WARNING: Your GCC compiler ($CXX) is less performant than clang (https://github.com/BITCONCHIO/bitconch/issues/7402). We suggest running the build script with -P or install your own clang and try again.${COLOR_NC}"
                    [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to proceed anyway? (y/n)?${COLOR_NC}" && read -p " " PROCEED
                    case $PROCEED in
                        "" ) echo "What would you like to do?";;
                        0 | true | [Yy]* ) break;;
...
function ensure-cmake() {
    ...
    if [[ ! -e "${CMAKE}" ]]; then
        ...
        && ./bootstrap --prefix=${BITCONCHIO_INSTALL_DIR} \
        && make -j${JOBS} \
        && make install \
        && cd .. \
