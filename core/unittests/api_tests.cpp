...
#include <bitconchio/chain/account_object.hpp>
#include <bitconchio/chain/contract_table_objects.hpp>
#include <bitconchio/chain/block_summary_object.hpp>
#include <bitconchio/chain/global_property_object.hpp>
#include <bitconchio/chain/generated_transaction_object.hpp>
#include <bitconchio/chain/wasm_interface.hpp>
...
bool is_access_violation(fc::unhandled_exception const & e) {
   try {
    ...
    catch (const bitconchio::chain::wasm_execution_error& e) {
       return true;
    } catch (...) {
...
bool is_wasm_execution_error(bitconchio::chain::wasm_execution_error const& e) {return true;}
...
BOOST_FIXTURE_TEST_CASE(action_receipt_tests, TESTER) { try {
   ...
   set_code( config::system_account_name, contracts::bitconchio_bios_wasm() );

   set_code( N(test), contracts::bitconchio_bios_wasm() );
   set_abi( N(test), contracts::bitconchio_bios_abi().data() );
	set_code( N(test), contracts::payloadless_wasm() );
...
BOOST_FIXTURE_TEST_CASE(action_tests, TESTER) { try {
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "assert_false", {} ),
                          bitconchio_assert_message_exception, bitconchio_assert_message_is("test_action::assert_false") );
   ...
   BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION( *this, "test_action", "read_action_to_0", raw_bytes), bitconchio::chain::wasm_execution_error,
         [](const bitconchio::chain::wasm_execution_error& e) {
   ...
   // test read_action_to_64k
   raw_bytes.resize(3);
	BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION( *this, "test_action", "read_action_to_64k", raw_bytes ), bitconchio::chain::wasm_execution_error,
         [](const bitconchio::chain::wasm_execution_error& e) {
            return expect_assert_message(e, "access violation");
         }
      );
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "test_current_time", fc::raw::pack(now) ),
                          bitconchio_assert_message_exception, bitconchio_assert_message_is("tmp == current_time()")     );
...
BOOST_FIXTURE_TEST_CASE(cf_action_tests, TESTER) { try {
      ...
      BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_transaction", "send_cf_action_fail", {} ),
                             bitconchio_assert_message_exception,
                             bitconchio_assert_message_is("context free actions cannot have authorizations") );
...
BOOST_FIXTURE_TEST_CASE(transaction_tests, TESTER) { try {
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION(*this, "test_transaction", "send_action_inline_fail", {}),
                          bitconchio_assert_message_exception,
                          bitconchio_assert_message_is("test_action::assert_false")                          );
   ...
   BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_transaction", "send_action_recurse", {}), bitconchio::chain::transaction_exception,
         [](const bitconchio::chain::transaction_exception& e) {
...
BOOST_FIXTURE_TEST_CASE(deferred_transaction_tests, TESTER) { try {
...

   {
      ...
      auth.accounts.push_back( permission_level_weight{{N(testapi), config::bitconchio_code_name}, 1} );
      ...
      // fail because testapi@additional permission is not unilaterally satisfied by testapi@bccio.code.
      dtt_action dtt_act3;
...
BOOST_AUTO_TEST_CASE(more_deferred_transaction_tests) { try {
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      bitconchio_assert_message_exception,
      bitconchio_assert_message_is("fail")
   );
...
BOOST_FIXTURE_TEST_CASE(multi_index_tests, TESTER) { try {
   ...
   auto check_failure = [this]( action_name a, const char* expected_error_msg ) {
      BOOST_CHECK_EXCEPTION(  push_action( N(testapi), a, N(testapi), {} ),
                              bitconchio_assert_message_exception,
                              bitconchio_assert_message_is( expected_error_msg )
      );
   };
...
BOOST_FIXTURE_TEST_CASE(permission_tests, TESTER) { try {
   ...
   CALL_TEST_FUNCTION( *this, "test_permission", "check_authorization",
      fc::raw::pack( check_auth {
         ...
         .permission = N(active),
         .pubkeys    = {
            public_key_type(string("BITCONCH7GfRtyDWWgxV88a5TRaYY59XmHptyfjsFmHHfioGNJtPjpSmGX"))
         }
      })
   );
...
/*************************************************************************************
 * bitconchio_assert_code_tests test cases
 *************************************************************************************/
BOOST_FIXTURE_TEST_CASE(bitconchio_assert_code_tests, TESTER) { try {
   produce_block();
   create_account( N(testapi) );
   produce_block();
   set_code(N(testapi), contracts::test_api_wasm() );
   ...
   const char* abi_string = R"=====(
{
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "test_assert_code", fc::raw::pack((uint64_t)42) ),
                          bitconchio_assert_code_exception, bitconchio_assert_code_is(42)                                        );
