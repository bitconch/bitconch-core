...
proxy::proxy( bitconchio::name self, bitconchio::name first_receiver, bitconchio::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      bitconchio::token::transfer_action a( "bccio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, bitconchio::ignore<std::vector<char>> ) {
