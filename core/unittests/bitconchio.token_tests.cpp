...
class bitconchio_token_tester : public tester {
public:

   bitconchio_token_tester() {
      produce_blocks( 2 );

      create_accounts( { N(alice), N(bob), N(carol), N(bccio.token) } );
      produce_blocks( 2 );
   ...
   fc::variant get_account( account_name acc, const string& symbolname)
   {
      auto symb = bitconchio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(bccio.token), acc, N(accounts), symbol_code );
   ...
   action_result create( account_name issuer,
      ...
      return push_action( N(bccio.token), N(create), mvo()
           ( "issuer", issuer)
...
BOOST_AUTO_TEST_SUITE(bitconchio_token_tests)

BOOST_FIXTURE_TEST_CASE( create_tests, bitconchio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_negative_max_supply, bitconchio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( symbol_already_exists, bitconchio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_supply, bitconchio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( create_max_decimals, bitconchio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( issue_tests, bitconchio_token_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( transfer_tests, bitconchio_token_tester ) try {
