...
#include <bitconchio/chain/permission_object.hpp>
#include <bitconchio/chain/authorization_manager.hpp>

#include <bitconchio/chain/resource_limits.hpp>
#include <bitconchio/chain/resource_limits_private.hpp>
...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for bitconchio reqauth action with alice's spending key
   chain.link_authority("alice", "bccio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "bccio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with bitconchio reqauth
   chain.unlink_authority("alice", "bccio", "reqauth");
   ...
   // Link authority for any bitconchio action with alice's scud key
   chain.link_authority("alice", "bccio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "bccio", "first",  "reqauth");
...
try {
   ...
   // Creating account with bccio. prefix with privileged account
   chain.create_account("bccio.test1");

   // Creating account with bccio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("bccio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'bccio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "bccio", "bccio.any", "reqauth" );
   chain.link_authority( "bob", "bccio", "bccio.any", "reqauth" );

   /// this should succeed because bitconchio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = bitconchio::chain::resource_limits::resource_usage_object;
   using by_owner = bitconchio::chain::resource_limits::by_owner;
