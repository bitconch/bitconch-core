...
#include <bitconchio/chain/fork_database.hpp>
...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(bccio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(bccio.token), contracts::bitconchio_token_wasm() );
   c.set_abi( N(bccio.token), contracts::bitconchio_token_abi().data() );
   c.produce_blocks(10);


   auto cr = c.push_action( N(bccio.token), N(create), N(bccio.token), mutable_variant_object()
              ("issuer",       "bccio" )
              ("maximum_supply", core_from_string("10000000.0000"))
      );

   cr = c.push_action( N(bccio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "bccio" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
      );

   cr = c.push_action( N(bccio.token), N(transfer), config::system_account_name, mutable_variant_object()
              ("from",     "bccio")
              ("to",       "dan" )
              ("quantity", core_from_string("100.0000"))
              ("memo", "")
