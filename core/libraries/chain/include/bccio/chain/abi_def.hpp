...
abi_def bccio_contract_abi(const abi_def& bccio_system_abi);
...
datastream<ST>& operator << (datastream<ST>& s, const bccio::chain::may_not_exist<T>& v) {
   raw::pack(s, v.value);
   return s;
}

template<typename ST, typename T>
datastream<ST>& operator >> (datastream<ST>& s, bccio::chain::may_not_exist<T>& v) {
   if (s.remaining())
...
template<typename T>
void to_variant(const bccio::chain::may_not_exist<T>& e, fc::variant& v) {
   to_variant( e.value, v);
}

template<typename T>
void from_variant(const fc::variant& v, bccio::chain::may_not_exist<T>& e) {
   from_variant( v, e.value );
}

} // namespace fc

FC_REFLECT( bccio::chain::type_def                         , (new_type_name)(type) )
FC_REFLECT( bccio::chain::field_def                        , (name)(type) )
FC_REFLECT( bccio::chain::struct_def                       , (name)(base)(fields) )
FC_REFLECT( bccio::chain::action_def                       , (name)(type)(ricardian_contract) )
FC_REFLECT( bccio::chain::table_def                        , (name)(index_type)(key_names)(key_types)(type) )
FC_REFLECT( bccio::chain::clause_pair                      , (id)(body) )
FC_REFLECT( bccio::chain::error_message                    , (error_code)(error_msg) )
FC_REFLECT( bccio::chain::variant_def                      , (name)(types) )
FC_REFLECT( bccio::chain::abi_def                          , (version)(types)(structs)(actions)(tables)
