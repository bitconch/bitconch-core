...
namespace bccio { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(bccio);
const static uint64_t null_account_name      = N(bccio.null);
const static uint64_t producers_account_name = N(bccio.prods);
...
const static uint64_t bccio_auth_scope       = N(bccio.auth);
const static uint64_t bccio_all_scope        = N(bccio.all);

const static uint64_t active_name = N(active);
const static uint64_t owner_name  = N(owner);
const static uint64_t bccio_any_name = N(bccio.any);
const static uint64_t bccio_code_name = N(bccio.code);
...
const static bccio::chain::wasm_interface::vm_type default_wasm_runtime = bccio::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace bccio::chain::config

constexpr uint64_t BCC_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / bccio::chain::config::percent_100;
}

template<typename Number>
Number BCC_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + bccio::chain::config::percent_100 - bccio::chain::config::percent_1)  / bccio::chain::config::percent_100;
}
