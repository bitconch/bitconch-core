...
#define N(X) bccio::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<bccio::chain::name> : private hash<uint64_t> {
      typedef bccio::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const bccio::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, bccio::chain::name& check);
} // fc


FC_REFLECT( bccio::chain::name, (value) )
