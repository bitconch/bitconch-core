 ...
 */
#include <bccio/chain/wast_to_wasm.hpp>
...
namespace bccio { namespace chain {
   ...
   { 
      ...
      if(parse_errors.size())
      {
         ...
         BCC_ASSERT( false, wasm_exception, "error parsing wast: ${msg}", ("msg",ss.str()) );
      }
      ...
      catch(const Serialization::FatalSerializationException& exception)
      {
         ...
         BCC_ASSERT( false, wasm_exception, "error converting to wasm: ${msg}", ("msg",ss.get()) );
