...
#include <bccio/chain/merkle.hpp>
...
namespace bccio { namespace chain {
   ...
   vector<block_header_extensions> block_header::validate_and_extract_header_extensions()const {
      ...
      for( size_t i = 0; i < header_extensions.size(); ++i ) {
         ...
         BCC_ASSERT( id >= id_type_lower_bound, invalid_block_header_extension,
         ...
         BCC_ASSERT( match, invalid_block_header_extension,
                     "Block header extension with id type ${id} is not supported",
                     ("id", id)
         );

         if( match->enforce_unique ) {
            BCC_ASSERT( i == 0 || id > id_type_lower_bound, invalid_block_header_extension,
